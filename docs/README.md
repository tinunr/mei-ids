# Documentation Index

This folder hosts all documentation for the repository. Start here and follow the module links.

Core docs:
- Architecture and system flow: [docs/architecture.md](docs/architecture.md)

Modules:
- Orchestrator (Spring Boot + Camel): [docs/modules/demo-orchestrator.md](docs/modules/demo-orchestrator.md)
- Notification service (NestJS + RMQ): [docs/modules/notification-service.md](docs/modules/notification-service.md)
- Moodle container: [docs/modules/moodle-container.md](docs/modules/moodle-container.md)
- RabbitMQ container: [docs/modules/rabbitmq-container.md](docs/modules/rabbitmq-container.md)
- Docker Compose environment: [docs/modules/docker-compose.md](docs/modules/docker-compose.md)

Demo and testing guides:
- Example flow: [docs/demo/EXEMPLO_COMPLETO_README.md](docs/demo/EXEMPLO_COMPLETO_README.md)
- Course sync flow: [docs/demo/SYNC_COURSE_README.md](docs/demo/SYNC_COURSE_README.md)
- Moodle service fix notes: [docs/demo/MOODLE_SERVICE_FIX.md](docs/demo/MOODLE_SERVICE_FIX.md)

Legacy and archived documents:
- Project slides: [docs/legacy/apresentacao-10-slides.md](docs/legacy/apresentacao-10-slides.md)
- Full report (archived): [docs/legacy/relatorio-final.md](docs/legacy/relatorio-final.md)
- Report copy: [docs/legacy/relatorio-final-copy.md](docs/legacy/relatorio-final-copy.md)
- Report copy 2: [docs/legacy/relatorio-final-copy-2.md](docs/legacy/relatorio-final-copy-2.md)
- Notification starter README: [docs/legacy/notification-readme.md](docs/legacy/notification-readme.md)
- Notes: [docs/legacy/demo.text](docs/legacy/demo.text)
- Empty slide stub: [docs/legacy/slade.md](docs/legacy/slade.md)
